...
class ConsoleRenderer:
    ...
    def render_summary(self) -> str:
        ...
        root_cause = improvement.get('root_cause', 'Thai digits require SHIFT key on Kedmanee layout')
    ...
    def render_scenarios_table(self) -> str:
        ...
        content.append("-" * 70)
        content.append(f"{'Scenario':<30} {'Time (min)':<12} {'Time (hrs)':<12} {'Status':<15}")
        content.append("-" * 70)
        
        for scenario_key, scenario in scenarios.items():
            status = "CURRENT" if scenario_key == "thai_kedmanee" else ""
            status = "OPTIMAL" if scenario_key == results['optimal_scenario'] else status
            
            content.append(f"{scenario['description']:<30} {scenario['total_cost_minutes']:<12.1f} {scenario['total_cost_hours']:<12.2f} {status:<15}")
        ...
        time_saved = improvement.get('time_saved_minutes', 'Unknown')
        efficiency_gain = improvement.get('efficiency_gain_percentage', 'Unknown')
        content.append(f"KEY FINDINGS: {time_saved} minutes saved per document ({efficiency_gain}% efficiency gain)")
        content.append("")
    ...
    def render_savings_analysis(self) -> str:
        ...
        content.append("-" * 70)
        content.append(f"{'Alternative Scenario':<30} {'Saved (min)':<12} {'Saved (%)':<12}")
        content.append("-" * 70)
        
        for scenario_key, saving in savings.items():
            content.append(f"{saving['description']:<30} {saving['time_saved_minutes']:<12.1f} {saving['percentage_saved']:<12.1f}")
    ...
    def render_impact_projections(self) -> str:
        ...
        content.append("-" * 70)
        content.append(f"Based on {impact['per_document_savings_minutes']} minutes saved per document")
        content.append("")
        content.append(f"{'Scale':<20} {'Annual Hours':<15} {'Annual Savings':<15}")
        content.append("-" * 50)
        
        for projection in impact['government_scale_projections']:
            content.append(f"{projection['scale']:<20} {projection['annual_hours_saved']:,.0f} hours     ${projection['annual_cost_savings']:,.0f}")
    ...
    def render_recommendations(self) -> str:
        ...
        content.append("-" * 40)
        content.append(f"PRIMARY: {recommendations['primary_recommendation']['action']}")
        content.append("")
    ...
    def render_quick_stats(self) -> str:
        ...
        # Safe extraction with defaults
        time_saved = key_findings.get('improvement', {}).get('time_saved_minutes', 'Unknown')
        efficiency_gain = key_findings.get('improvement', {}).get('efficiency_gain_percentage', 'Unknown')
        total_digits = document_stats.get('total_digits', 0)
        document_path = metadata.get('document_path', 'Unknown document')
        
        content = []
        content.append(f"⚡ Quick result: {time_saved} minutes saved per document ({efficiency_gain}% efficiency gain)")
        content.append(f"📄 Document: {document_path} ({total_digits} digits analyzed)")
        content.append(f"🎯 Solution: Switch to international digits (0-9) for immediate efficiency improvement")
    ...
    def render_json_summary(self) -> str:
        ...
        summary = {
            ...
            "efficiency_gain_percent": key_findings['improvement']['efficiency_gain_percentage'],
...
def render_json_to_console(json_data: Dict[str, Any], format_type: str = "comprehensive") -> str:
