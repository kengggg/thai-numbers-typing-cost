...
class MarkdownRenderer:
    ...
    def _render_header(self) -> str:
        ...
        try:
            generated_time = datetime.fromisoformat(generated_at).strftime("%B %d, %Y at %H:%M:%S")
        ...
        analysis_focus = metadata.get('analysis_focus', 'Thai digits vs International digits typing cost comparison')
...
- **Per-document efficiency gain:** {self.data.get('key_findings', {}).get('improvement', {}).get('efficiency_gain_percentage', 'Unknown')}%
...
**{recommendations.get('primary', 'Standardize on international digits (0-9) for all Thai government documents')}**
...
*Report generated by Thai Numbers Typing Cost Analysis Tool v{self.data.get('metadata', {}).get('tool_version', 'Unknown')}*
"""
        ...
        if generated_at:
            try:
                research_date = datetime.fromisoformat(generated_at).strftime("%B %d, %Y")
...
def main():
    ...
    print(f"Key insight: {analysis_data['key_findings']['improvement']['time_saved_minutes']} minutes saved per document")
