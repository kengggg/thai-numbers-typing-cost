...
class TextAnalyzer:
    ...
    def find_number_contexts(self) -> List[Dict]:
        ...
        for match in re.finditer(digit_pattern, self.text):
            ...
            contexts.append({
                ...
                'type': 'thai' if has_thai else 'international' if has_intl else 'mixed',
                'length': len(number)
            })
    ...
    def get_statistics(self) -> Dict:
        ...
        return {
            ...
            'number_sequences': {
                ...
                'avg_thai_length': sum(ctx['length'] for ctx in thai_sequences) / len(thai_sequences) if thai_sequences else 0,
                'avg_intl_length': sum(ctx['length'] for ctx in intl_sequences) / len(intl_sequences) if intl_sequences else 0
            },
    ...
    def print_report(self):
        ...
        print(f"  International digits (0-9): {stats['digit_counts']['international_digits']:,}")
        
        if stats['digit_analysis']['thai_digit_breakdown']:
            print(f"\n  Thai digit frequency:")
            for char, count in sorted(stats['digit_analysis']['thai_digit_breakdown'].items()):
        ...
        if stats['digit_analysis']['intl_digit_breakdown']:
            ...
            for char, count in sorted(stats['digit_analysis']['intl_digit_breakdown'].items()):
                unicode_code = stats['digit_analysis']['intl_digit_unicode'][char]
                print(f"    {char} ({unicode_code}): {count:,}")
        
        print(f"\nNUMBER SEQUENCES:")
        print(f"  Total number sequences: {stats['number_sequences']['total_sequences']:,}")
        print(f"  Thai number sequences: {stats['number_sequences']['thai_sequences']:,}")
        print(f"  International sequences: {stats['number_sequences']['intl_sequences']:,}")
        print(f"  Average Thai sequence length: {stats['number_sequences']['avg_thai_length']:.1f}")
        print(f"  Average International sequence length: {stats['number_sequences']['avg_intl_length']:.1f}")
