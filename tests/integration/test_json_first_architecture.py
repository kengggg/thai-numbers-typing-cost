...
class TestJSONDataIntegrity:
    ...
    def test_json_generation_to_markdown_pipeline(
        self, sample_thai_text_file, tmp_path
    ):
        ...
        assert (
            str(key_findings["improvement"]["time_saved_minutes"]) in markdown_content
        )
        assert (
            str(key_findings["improvement"]["efficiency_gain_percentage"])
            in markdown_content
        )
        ...
        assert (
            "# Thai Numbers Typing Cost Analysis - Research Report" in markdown_content
        )
    ...
    def test_json_schema_consistency(self, sample_thai_text_file):
        ...
        configs = [{"include_all_typists": False}, {"include_all_typists": True}]
        ...
        single_typist_data = generator.generate_comprehensive_analysis(
            include_all_typists=False
        )
        all_typists_data = generator.generate_comprehensive_analysis(
...
ตัวอักษรพิเศษ: ก่ ข่ ค่ ง่ จ๊ ฉ๋ ช์ ซํ ฏ๎ ฐ๏"""
        ...
        assert (
            "๐-๙" in saved_markdown
        )  # Thai digits range that appears in executive summary
        ...
        assert (
            "THAI NUMBERS TYPING COST ANALYSIS" in console_output
        )  # Should have Thai text
    ...
    def test_data_precision_preservation(self, sample_thai_text_file):
        ...
        def compare_numerical_values(original, reconstructed, path=""):
            if isinstance(original, dict):
                ...
                ), f"Precision lost at {path}: {original} != {reconstructed}"
                if isinstance(original, float):
                    # Check precision to reasonable decimal places
                    assert (
                        abs(original - reconstructed) < 1e-10
                    ), f"Float precision lost at {path}"
...
class TestCrossFormatConsistency:
    ...
    def test_key_metrics_consistency(self, sample_thai_text_file):
        ...
        expected_metrics = {
            ...
            "efficiency_gain_percentage": key_findings["improvement"][
        ...
        for metric_name, metric_value in expected_metrics.items():
            assert (
                str(metric_value) in markdown_content
            ), f"Missing {metric_name} in markdown"
...
class TestScalabilityAndPerformance:
    ...
    def test_large_document_json_generation(self, tmp_path):
        ...
        large_content = (
            """พระราชบัญญัติรัฐธรรมนูญแห่งราชอาณาจักรไทย พุทธศักราช ๒๕๖๐
...
"""
            * 100
        )  # Repeat 100 times to create larger document
...
def extract_json_schema(json_data):
    ...
    if "key_findings" in json_data and "improvement" in json_data["key_findings"]:
        schema["improvement_keys"] = set(
            json_data["key_findings"]["improvement"].keys()
        )
