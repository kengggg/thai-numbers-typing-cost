...
class TestMarkdownRenderer:
    ...
    def test_render_impact_analysis(self, sample_json_analysis_data):
        ...
        assert "| Scale | Documents/Day | Annual Hours Saved | Annual Cost Savings |" in impact
        assert "|-------|" in impact  # Table header separator
        
        # Check data integration 
        assert str(sample_json_analysis_data['impact_projections']['per_document_savings_minutes']) in impact
    ...
    def test_render_conclusion(self, sample_json_analysis_data):
        ...
        assert str(key_findings['improvement']['efficiency_gain_percentage']) in conclusion
    ...
    def test_save_to_file_creates_directories(self, sample_json_analysis_data, tmp_path):
...
class TestConsoleRenderer:
    ...
    def test_render_recommendations(self, sample_json_analysis_data):
        ...
        sample_json_analysis_data['recommendations'] = {
            'primary_recommendation': {
                'action': 'Switch to international digits'
            },
        ...
        assert "2. Train staff" in recommendations
    ...
    def test_render_json_summary(self, sample_json_analysis_data):
        ...
        # Check required keys
        required_keys = [
            'time_saved_minutes', 'efficiency_gain_percent',
            'current_time_minutes', 'optimal_time_minutes', 'root_cause'
        ]
        ...
        # Check data integration
        key_findings = sample_json_analysis_data['key_findings']
        assert parsed['time_saved_minutes'] == key_findings['improvement']['time_saved_minutes']
        assert parsed['efficiency_gain_percent'] == key_findings['improvement']['efficiency_gain_percentage']


class TestConvenienceFunctions:
    """Test suite for convenience functions."""
    
    def test_render_json_to_markdown_with_output(self, sample_json_analysis_data, tmp_path):
    ...
    def test_render_json_to_console_formats(self, sample_json_analysis_data, format_type, expected_content):
        ...
        if format_type == "comprehensive":
            sample_json_analysis_data.update({
                'analysis_results': {
                    'average': {
                        'scenarios': {'thai_kedmanee': {'description': 'Test', 'total_cost_minutes': 100.0, 'total_cost_hours': 1.67}},
                        'savings_analysis': {},
                        'optimal_scenario': 'thai_kedmanee'
                    }
                },
                'impact_projections': {'per_document_savings_minutes': 10.0, 'government_scale_projections': []},
                'recommendations': {'primary_recommendation': {'action': 'Test'}, 'implementation_steps': [], 'benefits': []}
            })
...
class TestMainFunctions:
    ...
    def test_markdown_renderer_main_with_args(self, sample_json_analysis_data, tmp_path, monkeypatch, capsys):
    ...
    def test_markdown_renderer_main_default_output(self, sample_json_analysis_data, tmp_path, monkeypatch, capsys):
...
class TestEdgeCases:
    ...
    def test_markdown_renderer_with_missing_data(self):
        ...
        minimal_data = {
            'metadata': {
                ...
                'document_stats': {'total_characters': 100, 'total_digits': 10, 'digit_percentage': 10.0}
            },
            'key_findings': {
                'current_state': {'time_minutes': 100.0, 'time_hours': 1.67},
                'optimal_state': {'time_minutes': 90.0, 'time_hours': 1.50},
                'improvement': {'time_saved_minutes': 10.0, 'efficiency_gain_percentage': 10.0, 'root_cause': 'SHIFT key'}
            },
            'research_questions': {},
            'impact_projections': {'per_document_savings_minutes': 10.0, 'government_scale_projections': []},
            'recommendations': {'primary_recommendation': {'action': 'Test'}, 'implementation_steps': [], 'benefits': []}
    ...
    def test_unicode_content_handling(self, tmp_path):
        ...
        unicode_data = {
            ...
            'research_questions': {},
            'impact_projections': {'per_document_savings_minutes': 10.0, 'government_scale_projections': []},
            'recommendations': {'primary_recommendation': {'action': 'ใช้ตัวเลขสากล'}, 'implementation_steps': [], 'benefits': []}  # Thai text
        }
