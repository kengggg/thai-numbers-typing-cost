...
class TestKeyboardLayoutAccuracy:
    ...
    def test_kedmanee_international_digit_finger_assignments(self, kedmanee_layout):
        ...
        for digit, (expected_hand, expected_finger) in standard_assignments.items():
            ...
            ), f"International digit {digit} not found in Kedmanee layout"
        ...
        ), "Finger assignment validation failed:\n" + "\n".join(validation_errors)
    ...
    def test_kedmanee_thai_digits_require_shift(self, kedmanee_layout, thai_digits):
        ...
        for digit in thai_digits:
            ...
            assert (
                key_info is not None
            ), f"Thai digit {digit} not found in Kedmanee layout"

            if not key_info.requires_shift:
                validation_errors.append(
                    f"Thai digit {digit} should require SHIFT but doesn't"
                )

            # All Thai digits should be on row 3 (number row)
            if key_info.row != 3:
                validation_errors.append(
                    f"Thai digit {digit} should be on row 3, but is on row {key_info.row}"
                )

        assert (
            not validation_errors
        ), "Thai digit SHIFT validation failed:\n" + "\n".join(validation_errors)

    def test_kedmanee_international_digits_no_shift(
        self, kedmanee_layout, international_digits
    ):
        ...
        for digit in international_digits:
            ...
            if key_info.requires_shift:
                validation_errors.append(
                    f"International digit {digit} should not require SHIFT but does"
                )

            # All international digits should be on row 3 (number row)
            if key_info.row != 3:
                validation_errors.append(
                    f"International digit {digit} should be on row 3, but is on row {key_info.row}"
                )

        assert (
            not validation_errors
        ), "International digit SHIFT validation failed:\n" + "\n".join(
            validation_errors
        )
    ...
    def test_pattajoti_thai_digits_no_shift(self, pattajoti_layout, thai_digits):
        ...
        for digit in thai_digits:
            ...
            assert (
                key_info is not None
            ), f"Thai digit {digit} not found in Pattajoti layout"

            if key_info.requires_shift:
                validation_errors.append(
                    f"Thai digit {digit} should not require SHIFT on Pattajoti but does"
                )
        ...
        assert (
            not validation_errors
        ), "Pattajoti Thai digit validation failed:\n" + "\n".join(validation_errors)
    ...
    def test_pattajoti_digit_order_accuracy(self, pattajoti_layout):
        ...
        assert (
            not validation_errors
        ), "Pattajoti digit order validation failed:\n" + "\n".join(validation_errors)
...
class TestLayoutCompleteness:
    ...
    def test_kedmanee_layout_completeness(self, kedmanee_layout):
        ...
        assert (
            layout_info["total_mapped_keys"] >= 60
        ), "Kedmanee should have at least 60 mapped keys"
        assert (
            layout_info["shifted_keys"] >= 20
        ), "Kedmanee should have at least 20 shifted keys"
        assert (
            layout_info["non_shifted_keys"] >= 40
        ), "Kedmanee should have at least 40 non-shifted keys"

        # Math should be consistent
        assert (
            layout_info["total_mapped_keys"]
            == layout_info["shifted_keys"] + layout_info["non_shifted_keys"]
        )
    ...
    def test_pattajoti_layout_completeness(self, pattajoti_layout):
        ...
        assert (
            layout_info["total_mapped_keys"] >= 70
        ), "Pattajoti should have at least 70 mapped keys"
        assert (
            layout_info["shifted_keys"] >= 5
        ), "Pattajoti should have some shifted keys"
        assert (
            layout_info["non_shifted_keys"] >= 65
        ), "Pattajoti should have mostly non-shifted keys"
    ...
    def test_essential_character_coverage(self, kedmanee_layout, pattajoti_layout):
        ...
        layouts = [("Kedmanee", kedmanee_layout), ("Pattajoti", pattajoti_layout)]
...
class TestLayoutConsistency:
    ...
    def test_international_digits_consistency(
        self, kedmanee_layout, pattajoti_layout, international_digits
    ):
        ...
        for digit in international_digits:
            ...
            assert (
                kedmanee_info is not None
            ), f"International digit {digit} missing in Kedmanee"
            assert (
                pattajoti_info is not None
            ), f"International digit {digit} missing in Pattajoti"

            # Finger assignments should be consistent for international digits
            if (
                kedmanee_info.hand != pattajoti_info.hand
                or kedmanee_info.finger != pattajoti_info.finger
            ):
        ...
        assert (
            not inconsistencies
        ), "International digit inconsistencies:\n" + "\n".join(inconsistencies)
    ...
    def test_row_assignments_validity(self, kedmanee_layout, pattajoti_layout):
        ...
        for layout_name, layout in layouts:
            ...
            assert (
                not invalid_rows
            ), f"{layout_name} has invalid row assignments: {invalid_rows}"
    ...
    def test_hand_assignments_validity(self, kedmanee_layout, pattajoti_layout):
        ...
        for layout_name, layout in layouts:
            ...
            assert (
                not invalid_hands
            ), f"{layout_name} has invalid hand assignments: {invalid_hands}"
    ...
    def test_finger_assignments_validity(self, kedmanee_layout, pattajoti_layout):
        ...
        for layout_name, layout in layouts:
            ...
            assert (
                not invalid_fingers
            ), f"{layout_name} has invalid finger assignments: {invalid_fingers}"


class TestTypingCostValidation:
    """Test suite for validating typing cost calculations."""

    def test_shift_penalty_application(
        self, kedmanee_layout, pattajoti_layout, thai_digits
    ):
    ...
    def test_international_digits_no_penalty(
        self, kedmanee_layout, pattajoti_layout, international_digits
    ):
    ...
    def test_typing_cost_mathematical_relationships(
        self, kedmanee_layout, pattajoti_layout
    ):
        ...
        for base_time in base_times:
            ...
            pat_thai_cost = pattajoti_layout.calculate_typing_cost(
                thai_digit, base_time
            )

            # International digit costs
            ked_intl_cost = kedmanee_layout.calculate_typing_cost(intl_digit, base_time)
            pat_intl_cost = pattajoti_layout.calculate_typing_cost(
                intl_digit, base_time
            )

            # Mathematical relationships
            assert (
                ked_thai_cost == base_time * 2.0
            ), "Kedmanee Thai digit should be 2x base time"
            assert (
                pat_thai_cost == base_time
            ), "Pattajoti Thai digit should be base time"
            assert (
                ked_intl_cost == base_time
            ), "Kedmanee international digit should be base time"
            assert (
                pat_intl_cost == base_time
            ), "Pattajoti international digit should be base time"

            # Cost comparisons
            assert (
                ked_thai_cost > pat_thai_cost
            ), "Kedmanee Thai should cost more than Pattajoti Thai"
            assert (
                ked_thai_cost > ked_intl_cost
            ), "Kedmanee Thai should cost more than Kedmanee international"
            assert (
                ked_intl_cost == pat_intl_cost
            ), "International digits should cost same on both layouts"
...
class TestSystemIntegration:
    ...
    def test_layout_explains_research_findings(self, kedmanee_layout, pattajoti_layout):
        ...
        assert (
            time_saved > 0
        ), "Pattajoti should save time over Kedmanee for Thai digits"
        assert (
            percentage_saved == 50.0
        ), "Should save exactly 50% (eliminating SHIFT penalty)"

        # The savings should be exactly the SHIFT penalty
        assert time_saved == base_time, "Time saved should equal base keystroke time"

    def test_layout_supports_government_document_typing(
        self, kedmanee_layout, pattajoti_layout
    ):
    ...
    def test_research_methodology_validation(self, kedmanee_layout, pattajoti_layout):
        ...
        assert (
            kedmanee_info.requires_shift
        ), "Research assumes Thai digits require SHIFT on Kedmanee"

        kedmanee_cost = kedmanee_layout.calculate_typing_cost(thai_digit, base_time)
        expected_cost = base_time * 2.0  # SHIFT penalty assumption

        assert (
            kedmanee_cost == expected_cost
        ...
        assert (
            not pattajoti_info.requires_shift
        ), "Research assumes Thai digits don't require SHIFT on Pattajoti"

        pattajoti_cost = pattajoti_layout.calculate_typing_cost(thai_digit, base_time)

        assert (
            pattajoti_cost == base_time
